...
const Slider = ({autoPlayInterval = 4000 }) => {
  ...
  useEffect(() => {
    fetch('http://localhost/Projects/ShoeStore-project/shoestore-backend/api/slider.php')
    .then(res => res.json())
      ...
      >
        {slides.map((img, index) => (
          <div key={index} className="w-full flex-shrink-0">
            <img
              src={`http://localhost/Projects/ShoeStore-project/shoestore-backend/uploads/image/${img.image}`}
              alt={`Slide ${index + 1}`}
