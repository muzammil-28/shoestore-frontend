...
function RelatedProducts({relProductId}) {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost/Projects/ShoeStore-project/shoestore-backend/api/related-products.php?rel_product_id=${relProductId}`)
        .then(res => res.json())
                        ...
                        relatedProducts.map((p, index) => (
                            <Link key={index} to={`/product-detail/${p.id}`}>
                                <div className='relative w-[220px] h-[310px] bg-white shadow-lg rounded-lg p-2 cursor-pointer hover:scale-[1.06]' data-aos="fade-up">
                                    ...
                                    <div>
                                        <img src={`http://localhost/Projects/ShoeStore-project/shoestore-backend/uploads/image/${p.image}`} alt={`Product image`} />
                                    </div>
                                    <div>
