...
function Header() {
    ...
    useEffect(() => {
        fetch("http://localhost/Projects/ShoeStore-project/shoestore-backend/api/navbar.php")
            .then(res => res.json())
    ...
    useEffect(() => {
        fetch(`http://localhost/Projects/ShoeStore-project/shoestore-backend/api/check-login.php`, { credentials: "include" })
            .then(res => res.json())
    ...
    const handleLogout = async () => {
        try {
            const logoutRes = await fetch(`http://localhost/Projects/ShoeStore-project/shoestore-backend/api/logout.php`,
                {
                    method: "POST",
                    credentials: "include"
                });
            ...
            <header>
                ...
                <div className={`md:hidden px-6 pb-4 space-y-4 overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-screen opacity-100 py-4" : "max-h-0 opacity-0 py-0"}`}>
                    ...
                    <ul className="space-y-2 mt-2">
                        {
                            menu.filter(item => item.parent_id === Number(0)).map(item => (
                                <li key={item.id} className="group">
                                    <Link to={`/${item.title}`} className="block text-gray-800 hover:text-blue-500 cursor-pointer mt-1">{item.title}</Link>
