...
function Cart() {
    ...
    useEffect(() => {
        fetch(`http://localhost/Projects/ShoeStore-project/shoestore-backend/api/cart-products.php`, { credentials: "include" })
            .then(res => res.json())
    ...
    const handleDelete = async (cartId) => {
        const DeleteRes = await fetch(
            `http://localhost/Projects/ShoeStore-project/shoestore-backend/api/delete-cart-products.php`,
            {
                method: "POST",
...

                {
                    cartProducts.map((product, index) => (
                        <div key={index} className="flex flex-col md:flex-row gap-4 border-b pb-4 mb-4">
                            ...
                            {/* Product Image */}
                            <img
                                src={`http://localhost/Projects/ShoeStore-project/shoestore-backend/uploads/image/${product.image}`}
                                alt="product"
