...
function ProductDetail() {
  ...
  useEffect(() => {
    fetch(`http://localhost/Projects/ShoeStore-project/shoestore-backend/api/product-detail.php?id=${id}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleAddToCart = async () => {
    try{
      const cartRes = await fetch(
        "http://localhost/Projects/ShoeStore-project/shoestore-backend/api/add-to-cart.php",
        {
          method : "POST",
          ...
          <div className='py-4 px-12 mt-5 flex flex-wrap gap-y-3 md:justify-between'>
            ...
            <div className='w-[40vw] md:w-100 border border-8 border-gray-200 rounded-2xl p-1'>
              ...
              <div className='w-full md:h-[350px] overflow-hidden group object-cover'>
                <img src={`http://localhost/Projects/ShoeStore-project/shoestore-backend/uploads/image/${mainImage}`} className='group-hover:scale-110 cursor-pointer' />
              </div>
              <div className="flex gap-3 mt-3 justify-center">
                  {
                    productDetail.image?.map((img, index) => (
                      <img 
                        key={index}
                        src={`http://localhost/Projects/ShoeStore-project/shoestore-backend/uploads/image/${img}`}
